...
void CacheSimulator::Run(std::string trace) {
    ...
    if (!source.is_open()) {
        throw std::invalid_argument("Couldn't find trace files. Make sure .exe is in same directory as the folder 'traces'.");
    }
...
void CacheSimulator::Output(bool showsets) {
    ...
    if (!l1Only) {
        std::cout << "k. L2 miss rate:              " + std::to_string((float)l2.read_misses / (float)l2.reads) << std::endl;
    }
    else {
    ...
    if (!l1Only) {
        if (Inc == 0) {
            std::cout << "m. total memory traffic:      " + std::to_string(l2.read_misses + l2.write_misses + l2.write_backs) << std::endl;
        }
        else if (Inc == 1) {
            std::cout << "m. total memory traffic:      " + std::to_string(l2.read_misses + l2.write_misses + l2.write_backs) << std::endl;
        }
        ...
        std::cout << "m. total memory traffic:      " + std::to_string(l1.read_misses + l1.write_misses + l1.write_backs) << std::endl;
    }
}